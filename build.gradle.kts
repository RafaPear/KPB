...
val kpbCliJar =
    tasks.register<Copy>("copykpbCliJar") {
        dependsOn(":kpb-cli:build")
        from(project(":kpb-cli").layout.buildDirectory.dir("libs"))
        into(layout.buildDirectory.dir("libs"))
    }

val kpbAppJar =
    tasks.register<Copy>("copykpbAppJar") {
        dependsOn(":kpb-app:build")
        from(project(":kpb-app").layout.buildDirectory.dir("libs"))
        into(layout.buildDirectory.dir("libs"))
    }

// Garante que os jars dos subprojetos aparecem no build do root
tasks.named("build") {
    dependsOn(kpbCliJar, kpbAppJar)
}
